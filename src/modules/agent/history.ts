import { Chat } from "openai/src/resources/beta/index.js";
...
export async function getHistory(
  ...
  const graph = await initGraph();
  const response = await graph.query<ChatbotResponse>(
    `
...
RETURN response.id AS id,
  ...
  return response ? response as ChatbotResponse[] : [];
}
// end::get[]
...
export async function saveHistory(
  ...
  const graph = await initGraph();
  const response = await graph.query<{ id: string }>(
    `
